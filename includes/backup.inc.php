...
require_once 'auth_checker.inc.php';
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        require_once 'file_model.inc.php';
        require_once 'file_contr.inc.php';
        require_once 'file_encryption.inc.php';
        ...
        if (is_recaptcha_invalid($secretKey, $recaptcha_response)) {
            ...
            if (isset($file["name"]) && file_exists_in_db($pdo, $_SESSION["user_id"], $file["name"])) {
                $errors["file_exist"] = "File exists, upload a different file";
            }
            if (is_key_length_invalid($encryption_key)) {
                $errors["invalid_key"] = "Encryption key must be exactly 32 characters long";
            }
        ...
        if (empty($errors)) {
            ...
            // load email template
            $file_email_upload_template = 'file-upload-template.html';
            ...
            $upload_directory = '../uploads/' . $_SESSION["user_id"] . '/';
